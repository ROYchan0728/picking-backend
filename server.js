...
// CORS — allow Vercel frontend (set FRONTEND_URL in Railway env vars)
...
app.post('/api/orders/upload', upload.single('file'), (req, res) => {
  ...
  try {
    ...
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      const num=cNum>=0?row[cNum]:null, desc=cDesc>=0?row[cDesc]:null, qty=cQty>=0?row[cQty]:null;
      if (!num||!desc||!qty) continue;
      items.push({ id:id++, numbering:String(num), description:String(desc), qty:Number(qty),
        user:cUser>=0&&row[cUser]?String(row[cUser]):'', uom:cUom>=0&&row[cUom]?String(row[cUom]):'', status:'pending' });
    }
    if (!items.length) return res.status(400).json({ error: 'No valid rows' });
    const data = readData();
